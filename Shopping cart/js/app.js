window.addEventListener("load", () => {
  ...
  const toggleShoppingControls = (enable) => {
    if (enable) {
      ...
      document.querySelector("#btn-purchased").classList.remove("disabled");
      document.querySelector("#btn-purchased").classList.add("hvr-pulse");
    } else {
      document.querySelector("#empty-cart").classList.add("disabled");
      document.querySelector("#btn-purchased").classList.add("disabled");
      document.querySelector("#btn-purchased").classList.remove("hvr-pulse");
    }
  };
  // Empty cart
  const emptyCart = () => {
    ...
    <td>
      <div class="row">
        <div class="d-flex justify-content-center quantity col-12 col-md-6">
          <input
            type="text"
            class="form-control rounded-sm text-dark border-info"
            type="number"
            min="1"
            max="10"
            step="1"
